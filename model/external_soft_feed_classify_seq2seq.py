...
from model.external_feed_decoder import ExternalFeedDecoder
...
class ExternalSoftFeedClassifySeq2Seq(nn.Module):
    ...
    def __init__(self, opt, rating_tokens_tensor=None):
        ...
        """
        super(ExternalSoftFeedClassifySeq2Seq, self).__init__()
        ...
        # self.detach_enc_logit_for_soft_feed = opt.detach_enc_logit_for_soft_feed
        self.detach_enc_logit_for_soft_feed = False

        self.rating_embedding_matrix = nn.Parameter(torch.zeros(opt.num_classes, self.emb_dim))
        init.uniform_(self.rating_embedding_matrix, -0.1, 0.1)
        ...
        if opt.classifier_type == "word_attn":
            ...
            self.enc_classifier = MaxPoolClassifier(self.memory_bank_size, opt.num_classes, opt.classifier_dropout,
    ...
    def forward(self, src, src_lens, trg, src_oov, max_num_oov, src_mask, trg_mask, rating, src_sent_positions, src_sent_nums, src_sent_mask):
        ...
        #print(self.enc_classifier.memory_bank_size)
        classifier_output = self.enc_classifier(classifier_memory_bank, src_mask, sent_memory_bank, src_sent_mask)
        ...
        if self.enc_classifier.ordinal:
            rating_dist = logit  # [batch, num_classes]
        else:
            rating_dist = torch.exp(logit)  # [batch, num_classes]


        # rating_embedding_matrix_expanded: [batch_size, num_classes, embed_dim]
        rating_embedding_matrix_expanded = self.rating_embedding_matrix.unsqueeze(0).expand(batch_size, self.num_classes, self.emb_dim)
        rating_dist = rating_dist.unsqueeze(1)  # [batch, 1, num_classes]
        if self.detach_enc_logit_for_soft_feed:
            rating_context = torch.bmm(rating_dist.detach(), rating_embedding_matrix_expanded)  # [batch, 1, embed_dim]
        else:
            rating_context = torch.bmm(rating_dist, rating_embedding_matrix_expanded)  # [batch, 1, embed_dim]
        rating_context = rating_context.squeeze(1)  # [batch, embed_dim]

        #print(rating_embedding.size())
        ...
        for t in range(max_target_length):
            ...
            decoder_dist, h_t_next, _, _, attn_dist, p_gen, coverage = \
                self.decoder(y_t, h_t, memory_banks, src_masks, max_num_oov, src_oov, coverage, decoder_memory_bank, rating_context, rating_memory_bank)
