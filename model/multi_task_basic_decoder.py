...
class MultiTaskBasicDecoder(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers, memory_bank_size, coverage_attn, copy_attn,
        ...
        super(MultiTaskBasicDecoder, self).__init__()
    ...
    def forward(self, y, h, memory_banks, src_masks, max_num_oovs, src_oov, coverage, decoder_memory_bank=None, rating_memory_bank=None):
        """
        :param y: [batch_size]
        ...
        word_context, word_attn_dist, coverage = self.word_attention_layer(last_layer_h_next, word_memory_bank,
                                                                           word_src_mask, coverage)
        # context: [batch_size, memory_bank_size], attn_dist: [batch_size, max_input_seq_len], coverage: [batch_size, max_input_seq_len]
        if self.hr_enc:
            sent_context, sent_attn_dist, _ = self.sent_attention_layer(last_layer_h_next, sent_memory_bank,
                                                                        sent_src_mask)
        ...
        if self.out_sentiment_context:
            ...
            sentiment_context, _, _ = self.sentiment_attention_layer(last_layer_h_next, word_memory_bank, word_src_mask,
                                                                     coverage=None)
