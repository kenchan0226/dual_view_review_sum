...
class ExternalFeedClassifySeq2Seq(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if opt.classifier_type == "word_attn":
            self.classifier = WordAttnClassifier(opt.query_hidden_size, self.memory_bank_size, opt.num_classes, opt.attn_mode, opt.classifier_dropout, opt.ordinal)
        elif opt.classifier_type == "max":
            self.classifier = MaxPoolClassifier(self.memory_bank_size, opt.num_classes, opt.classifier_dropout, opt.ordinal)
        elif opt.classifier_type == "word_attn_no_query":
            self.classifier = WordAttnNoQueryClassifier(self.memory_bank_size, opt.num_classes, opt.classifier_dropout,
                                                        opt.ordinal)
        elif opt.classifier_type == "word_multi_hop_attn":
            self.classifier = WordMultiHopAttnClassifier(opt.query_hidden_size, self.memory_bank_size, opt.num_classes,
    ...
    def init_embedding_weights(self):
        ...
        self.rating_embedding.weight.data.uniform_(-init_range, init_range)
    ...
    def forward(self, src, src_lens, trg, src_oov, max_num_oov, src_mask, trg_mask, rating, src_sent_positions, src_sent_nums, src_sent_mask):
        ...
        # classification
        #print(self.classifier.memory_bank_size)
        classifier_output = self.classifier(word_memory_bank, src_mask)
        ...
        rating_embedding = self.rating_embedding(rating)  # [batch_size, embed_dim]
        ...
        for t in range(max_target_length):
            ...
            decoder_dist, h_t_next, _, _, attn_dist, p_gen, coverage = \
                self.decoder(y_t, h_t, memory_banks, src_masks, max_num_oov, src_oov, coverage, decoder_memory_bank, rating_embedding)
