...
from model.rnn_decoder import HirEncRNNDecoder, RNNDecoder
...
class Seq2SeqModel(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if self.hr_enc:
            ...
            self.encoder = TwoLayerRNNEncoder(
                ...
                pad_token=self.pad_idx_src,
                dropout=self.dropout,
                reverse=self.separate_layer_enc_reverse
            )
        else:
    ...
    def forward(self, src, src_lens, trg, src_oov, max_num_oov, src_mask, trg_mask, rating, src_sent_positions, src_sent_nums, src_sent_mask):
        ...
        #return decoder_dist_all, h_t_next, attention_dist_all, word_encoder_final_state, coverage_all, word_memory_bank
        logit = None
...
class HirEncSeq2SeqModel(nn.Module):
    """Container module with an encoder, deocder, embeddings."""

    def __init__(self, opt):
        """Initialize model."""
        super(HirEncSeq2SeqModel, self).__init__()
    ...
    def forward(self, src, src_lens, trg, src_oov, max_num_oov, src_mask, src_sent_positions, src_sent_nums, src_sent_mask):
        ...
        # Encoding
        memory_banks, encoder_final_states = self.encoder(src, src_lens, src_mask, src_sent_positions, src_sent_nums)
        assert memory_banks[0].size() == torch.Size([batch_size, max_src_len, self.num_directions * self.encoder_size])
        assert encoder_final_states[0].size() == torch.Size([batch_size, self.num_directions * self.encoder_size])

        # Decoding
        # Initialize the decoder with the last word-level encoding state
        h_t_init = self.init_decoder_state(encoder_final_states[0])  # [dec_layers, batch_size, decoder_size]
        max_target_length = trg.size(1)
        #context = self.init_context(memory_bank)  # [batch, memory_bank_size]
        ...
        for t in range(max_target_length):
            ...
            decoder_dist, h_t_next, _, attn_dist, p_gen, coverage = \
                self.decoder(y_t, h_t, memory_banks, (src_mask, src_sent_mask), max_num_oov, src_oov, coverage, decoder_memory_bank)
            decoder_dist_all.append(decoder_dist.unsqueeze(1))  # [batch, 1, vocab_size]
            attention_dist_all.append(attn_dist.unsqueeze(1))  # [batch, 1, src_seq_len]
            if self.coverage_attn:
                coverage_all.append(coverage.unsqueeze(1))  # [batch, 1, src_seq_len]
        ...
        #return decoder_dist_all, h_t_next, attention_dist_all, encoder_final_states[0], coverage_all, memory_banks[0]
        return decoder_dist_all, h_t_next, attention_dist_all, encoder_final_states, coverage_all, memory_banks
