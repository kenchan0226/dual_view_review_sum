...
class MultiViewExternalSoftFeedClassifySeq2Seq(nn.Module):
    """Container module with an encoder, deocder, embeddings."""

    def __init__(self, opt, rating_tokens_tensor):
        """Initialize model.
        :param rating_tokens_tensor: a LongTensor, [5, rating_v_size], stores the top rating_v_size tokens' indexs of each rating score
        """
        super(MultiViewExternalSoftFeedClassifySeq2Seq, self).__init__()
    ...
    def forward(self, src, src_lens, trg, src_oov, max_num_oov, src_mask, trg_mask, rating, src_sent_positions, src_sent_nums, src_sent_mask):
        ...
        if isinstance(enc_classifier_output, tuple):
            ...
            enc_classify_attn_dist = enc_classifier_output[1]
        else:
            enc_logit = enc_classifier_output
            enc_classify_attn_dist = (None, None)
        ...
        if self.enc_classifier.ordinal:
            enc_rating_dist = enc_logit  # [batch, num_classes]
        else:
            enc_rating_dist = torch.exp(enc_logit)  # [batch, num_classes]

        #print(rating_dist.detach().cpu().numpy())
        ...
        if self.detach_enc_logit_for_soft_feed:
            enc_rating_context = torch.bmm(enc_rating_dist.detach(), rating_embedding_matrix_expanded)  # [batch, 1, embed_dim]
        else:
        ...
        for t in range(max_target_length):
            ...
            decoder_dist, h_t_next, _, sentiment_context, attn_dist, p_gen, coverage = \
                self.decoder(y_t, h_t, memory_banks, src_masks, max_num_oov, src_oov, coverage, decoder_memory_bank, enc_rating_context, rating_memory_bank)
        ...
        assert dec_classifier_input.size() == torch.Size((batch_size, max_target_length, self.num_directions * self.encoder_size))
